...
if [ ! -f "${mfccdir}/.done" ]; then
    steps/make_mfcc.sh --cmd "$train_cmd" --mfcc-config conf/mfcc_hires.conf --nj 10 ${data_hires_dir} exp/make_mfcc_hires/train $mfccdir
...
if [ ! -f "${mfccdir}/.done" ]; then
    steps/make_mfcc_pitch.sh --cmd "$train_cmd" --mfcc-config conf/mfcc_hires.conf --pitch-config conf/pitch.conf --nj 10 ${data_hires_pitch_dir} exp/make_mfcc_hires_pitch/train $mfccdir
